...
#' - If the argument `exclude_embeddings` (defaults to `TRUE`) is set to `TRUE`,
...
#'
#' Typically, you only need to pass the name of your densely-connected output
#' layer to `exclude_layers`, e.g. `exclude_layers = list("output_dense")`.
#'
#' # References
...
#' Kingma and Ba paper (in the formula just before Section 2.1), not the epsilon
#' in Algorithm 1 of the paper. It is used by the AdamW step.
#' Defaults to `config_epsilon()`.
#'
#' @param weight_decay
#' Float. Weight decay applied when using the AdamW step.
#' Defaults to `0.1`.
#'
#' @param clipnorm
#' Float. If set, the gradient of each weight is individually
...
#' @param use_ema
#' Boolean, defaults to `FALSE`.
#' If `TRUE`, exponential moving average (EMA) is applied.
#' EMA consists of computing an exponential moving average of the
...
#' Every `ema_overwrite_frequency` steps of iterations, we overwrite the model
...
#'
#' @param loss_scale_factor
#' Float or `NULL`. If a float, the scale factor will be multiplied with the
#' loss before computing gradients, and the inverse of the scale factor will be
#' multiplied by the gradients before updating variables. Useful for preventing
#' underflow during mixed precision training. Alternately,
...
#' steps, using the average value of the gradients since the last update.
#' This is known as "gradient accumulation". This can be useful when your batch
#' size is very small, in order to reduce gradient noise at each update step.
#' EMA frequency will look at "accumulated" iterations value (optimizer steps //
#' gradient_accumulation_steps). Learning rate schedules will look at "real"
#' iterations value (optimizer steps).
#'
#' @param name
#' String. The name to use for optimizer variables created by the optimizer.
#'
...
#'
#' @param adam_lr_ratio
#' Float, the ratio of the learning rate when using Adam to the main learning
#' rate. It is recommended to set it to `0.1`.
#'
#' @param momentum
#' Float, momentum used by the internal SGD step.
#'
#' @param ns_steps
...
function (learning_rate = 0.001, adam_beta_1 = 0.9, adam_beta_2 = 0.999,
    epsilon = config_epsilon(), weight_decay = 0.1, clipnorm = NULL,
    clipvalue = NULL, global_clipnorm = NULL, use_ema = FALSE,
    ema_momentum = 0.99, ema_overwrite_frequency = NULL,
    loss_scale_factor = NULL, gradient_accumulation_steps = NULL,
    name = "muon", exclude_layers = NULL, exclude_embeddings = TRUE,
    muon_a = 3.4445, muon_b = -4.775, muon_c = 2.0315,
    adam_lr_ratio = 0.1, momentum = 0.95, ns_steps = 6L,
    nesterov = TRUE, ...)
{
    args <- capture_args(list(ema_overwrite_frequency = as_integer,
        gradient_accumulation_steps = as_integer,
