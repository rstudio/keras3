...
tether: https://raw.githubusercontent.com/keras-team/keras-io/master/examples/nlp/neural_machine_translation_with_transformer.py
---
...
zip::zip_list(zipfile) # See what's in there
...
writeLines(sprintf("english: %s\nspanish: %s\n", df$english, df$spanish))
```
...
transformer
transformer |> compile("rmsprop", loss = "sparse_categorical_crossentropy", metrics =
                         "accuracy")
transformer |> fit(train_ds, epochs = epochs, validation_data = val_ds)
```
