...
tether: https:/raw.githubusercontent.com/keras-team/keras/master/guides/functional_api.py
---
...
title_input <- layer_input(
  shape = shape(NULL), name="title"
)  # Variable-length sequence of ints
body_input <- layer_input(
    shape = shape(NULL), name="body"
)  # Variable-length sequence of ints
tags_input <- layer_input(
...
custom_rnn <- new_layer_class(
  ...
  call = function(inputs, ...) {
    outputs <- list()
    state <- k_zeros(c(shape(inputs)[[1]], self$units))
    ...
    features <- k_stack(outputs, axis = 2)
    self$classifier(features)
  }
)

rnn_model <- custom_rnn()
out <- rnn_model(k_zeros(c(1, timesteps, input_dim)))
```
