...
a ground-up rebuild of the `keras` R package for Keras 3. It keeps
everything that was already great the same, and refines and simplifies
parts of the API based on lessons accumulated over the past few years.
...
the reference pages and guide are also available via R's built-in help
system.
...
-   We now take snapshots of the upstream documentation and API surface.
    With with each release, all R documentation is rebased on upstream
    updates with each release. This workflow ensures that all R
    documentation (guides, examples, vignettes, and reference pages) and
    R function signatures stay up-to-date with upstream. This
    snapshot+rebase functionality is also implemented in a standalone R
    package <https://github.com/t-kalinowski/doctether>
...
-   All examples and vignettes can now be evaluated and rendered during
    ...
    it into a release. The also means that all user facing examples now
    additionally serve as an extended suite of snapshot unit and
    integration tests.

    Previously, evaluating code in vignettes and examples was disabled
    due to CRAN restrictions. The CRAN restrictions are still in place,
    but the package now has an additional pre-rendering build-step,
    implemented as roxygen
    [hooks](https://github.com/rstudio/keras/blob/main/man/roxygen/meta.R)
    and vignette pre-rendering
    [`knit()`](https://github.com/rstudio/keras/blob/main/tools/knit.R)
    implementation.

Combined, these two features will make it substantially easier for the
Keras in R to maintain feature parity and up-to-date documentation with
the Python API to Keras.
...
Keras 3 also let's you incorporate any pre-existing Torch, Jax, or Flax
module as a standard Keras layer by using the appropriate wrapper,
letting you build atop existing projects with Keras.
