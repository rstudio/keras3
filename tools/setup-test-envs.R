...
tf_vers <- c("2.1", "2.2", "2.3", "2.4", "2.5", "2.6", "nightly")
tf_vers <- paste0(tf_vers, "-cpu")
py_vers <- c("3.6", "3.6", "3.7", "3.8", "3.8", "3.9", "3.9")
...
for (i in seq_along(tf_vers))
  try(keras::install_keras(
  # try(tensorflow::install_tensorflow(
    version = tf_vers[i],
...
reticulate:::pip_install(
  "/home/tomasz/.local/share/r-miniconda/envs/tf-2.4-cpu/bin/python",
  packages = c("tensorflow==2.4.*", keras:::default_extra_packages("2.4")),
  conda = FALSE, ignore_installed = TRUE)

reticulate:::pip_install(
  "/home/tomasz/.local/share/r-miniconda/envs/tf-2.5-cpu/bin/python",
...
# /home/tomasz/.local/share/r-miniconda/bin/conda run --name tf-2.5-cpu python -m pip install --upgrade 'tensorflow-cpu==2.5.*' tensorflow-hub scipy requests pyyaml 'Pillow<8.3' 'h5py pandas
