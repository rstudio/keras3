...
1) A [plot method](https://rstudio.github.io/keras/reference/plot.keras_training_history.html) for the Keras training history returned from `fit()`.

2) Real time visualization of model metrics within the RStudio IDE.

3) Integration with the [TensorBoard](https://www.tensorflow.org/get_started/summaries_and_tensorboard) visualization tool included with TensorFlow.

## Plotting History

The Keras `fit()` method returns an R object containing the training history, including the value of each metric at each epoch of the training run. You can visualization the training history using the `plot()` method. For example, here we compile and fit a model with the "accuracy" metric:
...
We can then plot the history as follows:
...
By default the history will be plotted using [ggplot2](http://ggplot2.tidyverse.org/) (a base graphics method is also available), include all specified metrics as well as the loss, and draw a smoothing line if there are 10 or more epochs. You can customize all of this behavior via various options of the [plot method](https://rstudio.github.io/keras/reference/plot.keras_training_history.html).
...
The `view_history` argument of the `fit()` method controls whether real time metrics are displayed. By default metrics are automatically displayed if one or more metrics are specified in the call to `compile()` and there is more than one training epoch. In addition, metrics are not shown if `verbose = 0`.

You can explicitly control whether metrics are displayed by specifying the `view_history` argument. You can also set a global session default using the `keras.view_history` option:

```{r}
# don't show history during this run
history <- model %>% fit(
  x_train, y_train, 
  epochs = 30, batch_size = 128, 
  view_history = FALSE,
...
# set global default to never show history
options(keras.view_history = FALSE)
```

Note that when `view_history` is `TRUE` metric history will be displayed even when not running within RStudio (in that case metrics will be displayed in an external web browser).
