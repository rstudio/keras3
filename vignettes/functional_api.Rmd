...
To re-use a model as a layer within another stack of layers, use the `as_layer()` function. When including a call to `as_layer()` inline be sure to surround it with parentheses so that the pipe operator (`%>%`) resolves it correctly:
...
merged_vector <- layer_concatenate(c(encoded_a, encoded_b), axis=-1)

```

The problem is that callable Python objects don't manifest as R functions. We can beat this to some extent by returning an R function with a "py_object" embedded. This works okay but every S3 method needs to be implemented for py_callable to delegate to the py_object. Even with this in place the obfuscating of the type info makes it hard to write other S3 methods or call inherits.
