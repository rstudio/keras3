...
To make use of a Keras backend you need to install it. For TensorFlow this can be done using the `install_tensorflow()` function. For other backends you should follow the [installation instructions](https://keras.io/#installation) on the Keras website.

To select an alternate backend, set the `KERAS_BACKEND` environment variable before loading the keras R package. For example:
