...
The TensorFlow backend is the recommended configuration for most users. If you intend to use Keras with TensorFlow then you should install it using the `install_tensorflow()` function. After a successful installation no further action or configuration is necessary since Keras will use TensorFlow by default.
