...
In this guide you learn how to customize what happens when you call `fit(model)`.
You see how to subclass `keras$Model` and create your own custom keras Model,
with it's own custom `train_step()` method.
...
generator <- keras_model_sequential(name = "generator") %>%
  ...
  layer_dense(7 * 7 * 128, input_shape = c(latent_dim)) %>%
  layer_activation_leaky_relu(alpha = 0.2) %>%
