...
You can inspect the various layers as well as the input and output tensors of a model using the `layers`, `inputs`, and `outputs` properites. For example:
...
```
[[1]]
Tensor("dense_1_input:0", shape=(?, 100), dtype=float32)
```
...
fit(model, x_train, y_train, batch_size = 16, epochs = 10)
score <- evaluate(model, x_test, y_test, batch_size = 16)
```
...
fit(model, x_train, y_train, batch_size = 16, epochs = 10)
score <- evaluate(evaluate, x_test, y_test, batch_size = 16)
```
