...
# fit with callbacks
fit(model, data, labels, callbacks = list(
...
# define model
model <- keras_model_sequential() %>% 
  layer_dense(units = 10, input_dim = c(784), init = 'uniform') %>% 
...
fit(model, X_train, Y_train, batch_size=128, epochs=20, verbose=0, callbacks= list(history))
