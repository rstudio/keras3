...
```{r}
callback_tensorboard(log_dir = run_dir())
```
...
Note that if you used the `run_dir()` function in `callback_tensorboard()`, you can use it again in the call to `tensorboard()` (by default `run_dir()` returns the same directory within a single R session):
