...
You should either use a distinct log directory for each training run or remove the log directory between runs. You can use the `run_dir()` function to easily create a directory per-run:
...
If you are using the `run_dir()` function to create a unique directory for each training run, you can either pass the `run_dir` to the `tensorboard()` function or use the `latest_run()` function to 
run tensorboad against the most recent run directory:
