...
You can run TensorBoard while you are training a model. To do this you can either:

1. Call the `tensorboard()` function within the directory you are training from; or

2. Specify the `launch = TRUE` parameter to the `callback_tensorboard()` function. For example;
    ...
    history <- model %>% fit(
      ...
      epochs = epochs,
      verbose = 1,
      callbacks = callback_tensorboard(log_dir = "logs/run_a", launch = TRUE),
      validation_split = 0.2
    )
    ```    

Note that when TensorBoard is first launched you won't see any scalar data plotted initially. This is because Keras writes TensorBoard data at the end of each training epoch, and the first epoch will likely not yet have completed. TensorBoard will automatically refresh it's display every 30 seconds during training.
...
You can use the `unique_dir` function if you want to record every training run in it's own directory:

```{r}
callback_tensorboard(log_dir = unique_dir("logs"))
```
