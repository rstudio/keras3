...
The first part of this guide covers saving and serialization for Sequential models and 
models built using the Functional API and for Sequential models. The saving and serialization 
...
model <- keras_model(inputs, outputs) # can't name the model
summary(model)
```
...
history <- model %>% fit(x_train, y_train, btach_size = 64, epochs = 1)
```
...
serving as well as TensorFlow implementations other than Python.
...
a Python dict that enables you to recreate the same model -- initialized from 
...
JSON string to store the config instead of a Python dict. This is useeful to save 
the config to disk.
...
Sequential models and Functional models are datastructures that represent a 
DAG of layers. As such, they can be safely serialized and deserialized.

A subclassed model differs in that it's not a datastructure, it's 
...
history <- model %>% fit(x_train, y_train, bach_size = 64, epochs = 1)
```
