...
[PlaidML](https://vertex.ai/blog/announcing-plaidml) is an open source portable deep learning engine that runs on most existing PC hardware with OpenCL-capable GPUs from NVIDIA, AMD, or Intel. PlaidML includes a Keras backend which you can use as described below.
...
For example, here's a [testthat](https://r-pkgs.had.co.nz/tests.html) utility function that can be used to skip a test when Keras isn't available:
