...
[PlaidML](http://vertex.ai/blog/announcing-plaidml) is an open source portable deep learning engine that runs on most existing PC hardware with OpenCL-capable GPUs from NVIDIA, AMD, or Intel. PlaidML includes a Keras backend which you can use as described below.
...
You installed PlaidML within a version of Python on your system (e.g. `/usr/bin/python`, `/anaconda3/bin/python`). If you aren't sure which version you used then execute `which python` from the shell where you installed PlaidML to see which version of Python was used.

When you load the Keras R package you should specify that this same version of Python be used via the `use_python()` function. You should furthermore call the `use_backend()` function to specify that you want to use the PlaidML backend. For example:
...
If you installed PlaidML in a virtual or conda environment you can alternatively specify that through the `use_virtualenv()` or `use_condaenv()` function. For example:
...
For example, here's a [testthat](http://r-pkgs.had.co.nz/tests.html) utility function that can be used to skip a test when Keras isn't available:
