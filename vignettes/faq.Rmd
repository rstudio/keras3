...
First, build and install PlaidML as described on the [project website](https://github.com/plaidml/plaidml). You must be sure that PlaidML is correctly installed and working before proceeding further!

You installed PlaidML within a version of Python on your system (e.g. `/usr/bin/python`, `/anaconda3/bin/python`). If you aren't sure which version you used then execute `which python` from a shell to see what your system's default version is. When you load the Keras R package you should specify that this version of Python be used via the `use_python()` function. You should furthermore call the `use_backend()` function to specify that you want to use the PlaidML backend. For example:
