...
- input_shape:
  ...
  have exactly 3 inputs channels. You can also omit this option if you
  would like to infer input_shape from an input_tensor. If you choose to
  include both input_tensor and input_shape then input_shape will be
  used if they match, if the shapes do not match then we will throw an
  error. E.g. `(160, 160, 3)` would be one valid value.
...
- minimalistic:
  ...
  In addition to large and small models this module also contains
  so-called minimalistic models, these models have the same per-layer
  dimensions characteristic as MobilenetV3 however, they don't utilize
  any of the advanced blocks (squeeze-and-excite units, hard-swish, and
  5x5 convolutions). While these models are less efficient on CPU, they
...
- weights:
  ...
  String, one of `NULL` (random initialization), `"imagenet"`
...
- classes:
  ...
  Integer, optional number of classes to classify images into, only to
...
- include_preprocessing:
  ...
  Boolean, whether to include the preprocessing layer (`Rescaling`) at
  the bottom of the network. Defaults to `TRUE`.
...
- [Searching for MobileNetV3](https://arxiv.org/pdf/1905.02244) (ICCV
  2019)
...
For MobileNetV3, by default input preprocessing is included as a part of
the model (as a `Rescaling` layer), and thus
[`application_preprocess_inputs()`](https://keras3.posit.co/reference/process_utils.md)
is actually a pass-through function. In this use case, MobileNetV3
...
MobileNetV3 models expect their inputs to be float tensors of pixels
with values in the `[-1, 1]` range.

## Call Arguments

- `inputs`: A floating point `numpy.array` or backend-native tensor, 4D
  with 3 color channels, with values in the range `[0, 255]` if
  `include_preprocessing` is `TRUE` and in the range `[-1, 1]`
  otherwise.
