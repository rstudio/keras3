...
- call_endpoint:
  ...
  [`call()`](https://rdrr.io/r/base/call.html) method of the reloaded
  layer. If the SavedModel was created via
  [`export_savedmodel()`](https://rdrr.io/pkg/tensorflow/man/export_savedmodel.html),
  then the default endpoint name is `'serve'`. In other cases it may be
  named `'serving_default'`.
...
## Examples
    ...
    ## Captures:
    ##   133747420180176: TensorSpec(shape=(), dtype=tf.resource, name=None)
    ##   133747420180752: TensorSpec(shape=(), dtype=tf.resource, name=None)
...
- Only call endpoints with a single `inputs` tensor argument (which may
  optionally be a named list/list of tensors) are supported. For
  endpoints with multiple separate input tensor arguments, consider
  subclassing `layer_tfsm` and implementing a
  [`call()`](https://rdrr.io/r/base/call.html) method with a custom
  signature.
...
- If you need training-time behavior to differ from inference-time
  ...
  artifact, and provide its name to the `layer_tfsm` via the
