...
`layer_torch_module_wrapper()` is only compatible with the PyTorch
backend and cannot be used with the TensorFlow or JAX backends.
...
- module:
  ...
  `torch.nn.Module` instance. If it's a `LazyModule` instance, then its
  parameters must be initialized before passing the instance to
  `layer_torch_module_wrapper` (e.g. by calling it once).
...
Here's an example of how the `layer_torch_module_wrapper()` can be used
with vanilla PyTorch modules.
