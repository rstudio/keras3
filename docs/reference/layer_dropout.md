...
Note that the `Dropout` layer only applies when `training` is set to
`TRUE` in [`call()`](https://rdrr.io/r/base/call.html), such that no
values are dropped during inference. When using `model.fit`, `training`
will be appropriately set to `TRUE` automatically. In other contexts,
you can set the argument explicitly to `TRUE` when calling the layer.
...
- noise_shape:
  ...
  1D integer tensor representing the shape of the binary dropout mask
  that will be multiplied with the input. For instance, if your inputs
  have shape `(batch_size, timesteps, features)` and you want the
