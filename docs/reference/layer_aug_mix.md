...
num_chains different ways, with each chain consisting of chain_depth
augmentations.
...
- num_chains:
  ...
  an integer representing the number of different chains to be mixed,
  defaults to 3.

- chain_depth:

  an integer representing the maximum number of transformations to be
  applied in each chain. The actual number of transformations in each
  chain will be sampled randomly from the range `[0, `chain_depth`]`.
  Defaults to 3.

- factor:

  The strength of the augmentation as a normalized value between 0
  and 1. Default is 0.3.

- alpha:

  a float value used as the probability coefficients for the Beta and
...
- interpolation:
  ...
  The interpolation method to use for resizing operations. Options
  include `"nearest"`, `"bilinear"`. Default is `"bilinear"`.
