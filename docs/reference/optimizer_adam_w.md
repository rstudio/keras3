...
AdamW optimization is a stochastic gradient descent method that is based
on adaptive estimation of first-order and second-order moments with an
added method to decay weights per the techniques discussed in the paper,
'Decoupled Weight Decay Regularization' by [Loshchilov, Hutter et al.,
2019](https://arxiv.org/abs/1711.05101).

According to [Kingma et al., 2014](https://arxiv.org/abs/1412.6980), the
underlying Adam method is "*computationally efficient, has little memory
...
- [Reddi et al., 2018](https://openreview.net/pdf?id=ryQu7f-RZ) for
  `amsgrad`.
