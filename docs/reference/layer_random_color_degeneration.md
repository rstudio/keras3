...
- factor:
  ...
  to which the image sharpness is impacted. `factor=0.0` makes this
  layer perform a no-op operation, while a value of 1.0 uses the
  degenerated result entirely. Values between 0 and 1 result in linear
  interpolation between the original image and the sharpened image.
...
- value_range:
  ...
  Typically, this would be `(0, 1)` for normalized images or `(0, 255)`
  for raw images.
