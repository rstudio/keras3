...
This layer resizes an image input to a target height and width. The
input should be a 4D (batched) or 3D (unbatched) tensor in
`"channels_last"` format. Input pixel values can be of any range (e.g.
`[0., 1.)` or `[0, 255]`).
...
- interpolation:
  ...
  String, the interpolation method. Supports `"bilinear"`, `"nearest"`,
