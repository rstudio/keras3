...
- bin_boundaries:
  ...
  A list of bin boundaries. The leftmost and rightmost bins will always
  extend to `-Inf` and `Inf`, so `bin_boundaries = c(0, 1, 2)` generates
  bins `(-Inf, 0)`, `[0, 1)`, `[1, 2)`, and `[2, +Inf)`. If this option
  is set, [`adapt()`](https://keras3.posit.co/reference/adapt.md) should
  not be called.

- num_bins:

  The integer number of bins to compute. If this option is set,
  `bin_boundaries` should not be set and
  [`adapt()`](https://keras3.posit.co/reference/adapt.md) should be
  called to learn the bin boundaries.

- epsilon:

  Error tolerance, typically a small fraction close to zero (e.g. 0.01).
  Higher values of epsilon increase the quantile approximation, and
