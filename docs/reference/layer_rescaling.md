...
This layer rescales every value of an input (often an image) by
multiplying by `scale` and adding `offset`.

For instance:

1.  To rescale an input in the `[0, 255]` range to be in the `[0, 1]`
    range, you would pass `scale=1./255`.

2.  To rescale an input in the `[0, 255]` range to be in the `[-1, 1]`
    range, you would pass `scale=1./127.5, offset=-1`.

The rescaling is applied both during training and inference. Inputs can
...
- scale:
  ...
  scalar, the same scale is applied to every feature or channel; if a
  vector or array, scaling is applied per channel.

- offset:

  Numeric scalar, vector, or array. The offset to apply to the inputs.
  If scalar, the same offset is applied to every feature or channel; if
  a vector or array, the shift is applied per channel.
