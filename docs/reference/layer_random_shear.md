...
- x_factor:
  ...
  A tuple of two floats. For each augmented image, a value is sampled
  from the provided range. If a float is passed, the range is
  ...
  way across the image. All provided values should be positive.
...
- fill_value:
  ...
  A float representing the value to be filled outside the boundaries
