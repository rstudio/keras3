...
- strides:
  ...
  int or tuple/list of 2 integers, specifying the stride length of the
...
- dilation_rate:
  ...
  int or tuple/list of 2 integers, specifying the dilation rate to use
...
- If `return_sequences`: 5D tensor with shape:
  `(samples, timesteps, filters, new_rows, new_cols)` if
  data_format='channels_first' or shape:
  `(samples, timesteps, new_rows, new_cols, filters)` if
  `data_format='channels_last'`.

- Else, 4D tensor with shape: `(samples, filters, new_rows, new_cols)`
  if `data_format='channels_first'` or shape:
  `(samples, new_rows, new_cols, filters)` if
  `data_format='channels_last'`.
