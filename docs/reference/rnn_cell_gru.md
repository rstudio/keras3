...
whereas [`layer_gru()`](https://keras3.posit.co/reference/layer_gru.md)
...
- reset_after:
  ...
  multiplication). `FALSE` = `"before"`, `TRUE` = `"after"` (default and
  cuDNN compatible).
...
- `training`: Python boolean indicating whether the layer should behave
  in training mode or in inference mode. Only relevant when `dropout` or
