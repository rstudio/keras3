...
- output_padding:
  ...
  Scalar integer or vector of two integers. Amount of padding to add to
  the height and width of the output tensor. Each element must be
  smaller than the corresponding stride. When `NULL` (default) the
  output size is inferred.
...
- dilation_rate:
  ...
  Scalar integer or vector of 2 integers specifying the dilation rate.
