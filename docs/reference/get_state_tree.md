...
- value_format:
  ...
  One of `"backend_tensor"`, `"numpy_array"`, `"array"`. The kind of
  array to return as the leaves of the nested state tree.

## Value

A named list containing the nested representations of the requested
variables. The names are the variable names, and the values are the
corresponding nested named lists.
