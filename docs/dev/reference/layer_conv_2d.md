...
created and added to the outputs. Finally, if `activation` is not
`NULL`, it is applied to the outputs as well.
...
`Conv2D` operations may show larger variations. Due to the large number
of element-wise multiplications and additions in convolution operations,
especially with large inputs or kernel sizes, accumulated floating-point
differences can exceed this 1e-7 threshold. These variations are
...
- kernel_size:
  ...
  int or list of 2 integer, specifying the size of the convolution
  window.
