...
Applies adaptive average pooling to volumetric data so that the output
has the target spatial size specified by `output_size`, regardless of
the input spatial size. The kernel size and stride are computed
