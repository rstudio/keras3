...
The values are drawn from a Beta distribution parametrized by alpha and
beta.
...
- alpha:
  ...
  Float or an array of floats representing the first parameter alpha.
  Must be broadcastable with `beta` and `shape`.

- beta:

  Float or an array of floats representing the second parameter beta.
  Must be broadcastable with `alpha` and `shape`.
