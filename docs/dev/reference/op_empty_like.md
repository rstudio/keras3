...
- x:
  ...
  Input tensor whose shape and dtype are used.

- dtype:

  Optional output dtype. If `NULL`, the dtype of `x` is used.

## Value

A tensor with the shape and requested dtype of `x`. Its contents are
arbitrary.
