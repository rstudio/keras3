...
``` r
op_cholesky_inverse(x, upper = FALSE)
```
...
- x:
  ...
  Lower- or upper-triangular Cholesky factor with shape `(..., M, M)`.

- upper:

  Whether `x` is upper triangular. If `FALSE`, `x` is treated as lower
  triangular.

## Value

A tensor of shape `(..., M, M)` containing the inverse of the matrix
represented by `x`.
...
## Examples
    ...
    ## tf.Tensor(
    ## [[0.25       0.        ]
    ##  [0.         0.11111112]], shape=(2, 2), dtype=float32)
