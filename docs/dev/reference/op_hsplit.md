...
``` r
op_hsplit(x, indices_or_sections)
```
...
- indices_or_sections:
  ...
  If an integer `N`, split into `N` equal sections along the second
  axis, or the first axis for a one-dimensional tensor. If a
  one-dimensional vector of sorted integers, its entries give the
  1-based positions at which to split that axis.
