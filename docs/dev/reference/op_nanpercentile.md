...
``` r
op_nanpercentile(x, q, axis = NULL, method = "linear", keepdims = FALSE)
```
...
- axis:
  ...
  Axis or axes along which to compute percentiles. By default, the input
  is flattened.
...
A tensor containing the requested percentiles, with `NaN` values
omitted.
