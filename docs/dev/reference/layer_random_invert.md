...
complementary values. Images that are not selected for inversion remain
unchanged.
...
- factor:
  ...
  probability of inverting the image colors. If a tuple is provided, the
  value is sampled between the two values for each image, where
  `factor[0]` is the minimum and `factor[1]` is the maximum probability.
  If a single float is provided, a value between `0.0` and the provided
  float is sampled. Defaults to `(0, 1)`.

- value_range:

  a tuple or a list of two elements. The first value represents the
  lower bound for values in passed images, the second represents the
