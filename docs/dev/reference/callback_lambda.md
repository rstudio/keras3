...
- ...:
  ...
  Any function in
  [`Callback()`](https://keras3.posit.co/dev/reference/Callback.md) that
  you want to override by passing `function_name = function`. For
  example, `callback_lambda(.., on_train_end = train_end_fn)`. The
  custom function needs to have the same arguments as the ones defined
  in [`Callback()`](https://keras3.posit.co/dev/reference/Callback.md).
