...
``` r
op_segment_prod(data, segment_ids, num_segments = NULL, sorted = FALSE)
```
...
- segment_ids:
  ...
  A 1-D tensor containing 1-based segment indices for elements in
  `data`. Its length must match the first dimension of `data`.
...
A tensor containing the product of values for each segment.
...
#> tf.Tensor([2.e+00 2.e+02 2.e+04], shape=(3), dtype=float32)
```
