...
input over a 3D spatial (or temporal) dimension (width,height and depth)
to produce a tensor of outputs. If `use_bias` is `TRUE`, a bias vector
is created and added to the outputs. Finally, if `activation` is not
...
- kernel_size:
  ...
  int or list of 3 integer, specifying the size of the convolution
  window.

- strides:

  int or list of 3 integer, specifying the stride length of the
...
- dilation_rate:
  ...
  int or vector of 3 ints, specifying the dilation rate to use for
  dilated convolution.
