...
``` r
op_nanmedian(x, axis = NULL, keepdims = FALSE)
```
...
- axis:
  ...
  Axis or axes along which to reduce. By default, the input is
  flattened.
...
A tensor containing median values. If all values along a reduced axis
are `NaN`, the corresponding result is `NaN`.
