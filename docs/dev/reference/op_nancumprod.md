...
``` r
op_nancumprod(x, axis = NULL, dtype = NULL)
```
...
- axis:
  ...
  Axis along which to compute cumulative products. By default, the input
  is flattened.
...
An output tensor with the same shape as `x` when `axis` is supplied, or
a flattened tensor otherwise.
