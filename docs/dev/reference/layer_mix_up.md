...
``` r
layer_mix_up(object, alpha = 0.2, data_format = NULL, seed = NULL, ...)
```
...
- alpha:
  ...
  Float between 0 and 1. Controls the blending strength. Smaller values
  mean less mixing, while larger values allow for more blending between
  images. Defaults to 0.2, recommended for ImageNet1k classification.
