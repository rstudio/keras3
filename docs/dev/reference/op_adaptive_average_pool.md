...
Applies adaptive average pooling that automatically computes the kernel
size and stride needed to produce `output_size`. This is useful when a
fixed output size is required regardless of input size, as in global
feature extraction for models such as ResNet.

## Usage

``` r
op_adaptive_average_pool(inputs, output_size, data_format = NULL)
```
...
- inputs:
  ...
  A rank-4 input tensor. With `data_format = "channels_last"`, the shape
  is `(batch, height, width, channels)`; with
  `data_format = "channels_first"`, it is
  `(batch, channels, height, width)`.

- output_size:

  Integer or integer vector of length two specifying the output height
  and width. A scalar uses the same size for both dimensions.
...
A rank-4 tensor with the requested spatial dimensions.
...
#>    [1. 1. 1.]
#>    [1. 1. 1.]
#>    [1. 1. 1.]]]], shape=(2, 7, 7, 3), dtype=float32)
```
