...
Unlike [`op_cast()`](https://keras3.posit.co/dev/reference/op_cast.md),
this function reinterprets the existing bytes rather than converting
values.

## Usage

``` r
op_view(x, dtype = NULL)
```
...
- dtype:
  ...
  Data type of the returned view. If `NULL`, returns a view with the
  input data type.
...
## Examples
    ...
    ## tf.Tensor([1.e-45 3.e-45], shape=(2), dtype=float32)
