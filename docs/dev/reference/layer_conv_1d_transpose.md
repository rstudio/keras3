...
- output_padding:
  ...
  Scalar integer. Amount of padding to add to the output length. Must be
  less than the stride. When `NULL` (default) the output size is
  inferred.
...
- dilation_rate:
  ...
  Scalar integer. Specifies the dilation rate. Values other than 1
  currently require `strides = 1` and rates greater than 1 are not
  supported.
