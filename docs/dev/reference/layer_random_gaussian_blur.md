...
- factor:
  ...
  aggressive blurring available. If a tuple is used, a `factor` is
  sampled between the two values for every image augmented. If a single
  float is used, a value between `0.0` and the passed float is sampled.
  Default is 1.0.

- kernel_size:

  Integer. Size of the Gaussian kernel used for blurring. Must be an odd
  integer. Default is 3.

- sigma:

  Float or tuple of two floats. Standard deviation of the Gaussian
  kernel. Controls the intensity of the blur. If a tuple is provided, a
  value is sampled between the two for each image. Default is 1.0.
