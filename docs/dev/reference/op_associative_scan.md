...
[`op_scan()`](https://keras3.posit.co/dev/reference/op_scan.md), with
the key difference that `op_associative_scan()` is a parallel
implementation with potentially significant performance benefits,
especially when jit compiled. The catch is that it can only be used when
`f` is a binary associative operation (i.e. it must verify
`f(a, f(b, c)) == f(f(a, b), c)`).
