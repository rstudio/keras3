...
are neural networks which share weights between two or more sister
networks, each producing embedding vectors of its respective inputs.

In supervised similarity learning, the networks are then trained to
...
segmentation of the training inputs.
...
classes. For example, digit `0` needs to be differentiated from the rest
of the digits (`1` through `9`), digit `1` from `0` and `2` through `9`,
and so on. To carry this out, we select N random images from class A
(for example, digit `0`) and pair them with N random images from another
class B (for example, digit `1`). We repeat this process for all classes
of digits.
...
We get 60,000 training pairs. Each pair contains two images, and each
...
![plot of chunk
unnamed-chunk-10](mnist_siamese_graph/unnamed-chunk-10-1.png)
...
![plot of chunk
unnamed-chunk-11](mnist_siamese_graph/unnamed-chunk-11-1.png)
...
![plot of chunk
unnamed-chunk-12](mnist_siamese_graph/unnamed-chunk-12-1.png)
...
produces embeddings. A `Lambda` layer then merges them using [Euclidean
...
![plot of chunk
unnamed-chunk-18](mnist_siamese_graph/unnamed-chunk-18-1.png)
...
![plot of chunk
unnamed-chunk-20](mnist_siamese_graph/unnamed-chunk-20-1.png)
