...
[`fit()`](https://generics.r-lib.org/reference/fit.html). This is
covered in the guide [Customizing what happens in
`fit()`](https://keras3.posit.co/articles/custom_train_step_in_tensorflow.md).
...
```
    ...
    ## Start of epoch  2
    ## Training loss (for one batch) at step 200: 0.5921
    ## Seen so far: 12800 samples
    ## Training loss (for one batch) at step 400: 0.2057
    ## Seen so far: 25600 samples
    ## Training loss (for one batch) at step 600: 0.4113
    ## Seen so far: 38400 samples
    ## Start of epoch  3
    ## Training loss (for one batch) at step 200: 0.3643
    ## Seen so far: 12800 samples
    ## Training loss (for one batch) at step 400: 0.4975
    ## Seen so far: 25600 samples
    ## Training loss (for one batch) at step 600: 0.2268
    ## Seen so far: 38400 samples
...
Let’s use this knowledge to compute `SparseCategoricalAccuracy` on
...
```
    ...
    ## Start of epoch  2
    ## Training loss (for one batch) at step 200: 0.8116
    ## Seen so far: 12800 samples
    ## Training loss (for one batch) at step 400: 0.7084
    ## Seen so far: 25600 samples
    ## Training loss (for one batch) at step 600: 0.4928
    ## Seen so far: 38400 samples
    ## Training acc over epoch: 0.8861
...
```
    ...
    ## Time difference of 43.37494 secs

## Speeding-up your training step with `tf_function()`

The default runtime in TensorFlow 2 is [eager
execution](https://www.tensorflow.org/guide/eager). As such, our
...
input. Just add a `@tf.function` decorator on it, like this:
...
```
    ...
    ## Time difference of 0.4359508 secs
...
forward pass by layers that call `self$add_loss(value)`. The resulting
list of scalar loss values are available via the property `model$losses`
...
Since our discriminator and generator are convnets, you’re going to want
to run this code on a GPU.
