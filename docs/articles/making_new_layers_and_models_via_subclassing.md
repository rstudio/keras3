...
- The `add_loss()` method
- The `training` argument in
  [`call()`](https://rdrr.io/r/base/call.html)
- The `mask` argument in [`call()`](https://rdrr.io/r/base/call.html)
...
You would use a layer by calling it on some tensor input(s), much like
an R function.
...
The [`call()`](https://rdrr.io/r/base/call.html) method of your layer
will automatically run build the first time it is called. You now have a
layer that’s lazy and thus easier to use:
...
creating weights only once from using weights in every call.
...
leave it to the first [`call()`](https://rdrr.io/r/base/call.html) to
trigger building their weights.
...
As long as a layer only uses APIs from the `ops` namespace (ie. using
functions starting with `op_`), (or other Keras namespaces such as
`activations_*`, `random_*`, or `layer_*`), then it can be used with any
backend – TensorFlow, JAX, or PyTorch.

All layers you’ve seen so far in this guide work with all Keras
backends.

The `ops` namespace gives you access to:

- The NumPy API, e.g. `op_matmul`, `op_sum`, `op_reshape`, `op_stack`,
  etc.
- Neural networks-specific APIs such as `op_softmax`, `op_conv`,
  `op_binary_crossentropy`, `op_relu`, etc.

You can also use backend-native APIs in your layers (such as `tf$nn`
functions), but if you do this, then your layer will only be usable with
the backend in question. For instance, you could write the following
JAX-specific layer using `jax$numpy`:
...
Because cross-backend compatibility is a tremendously useful property,
we strongly recommend that you seek to always make your layers
backend-agnostic by leveraging only Keras APIs.

## The `add_loss()` method

When writing the [`call()`](https://rdrr.io/r/base/call.html) method of
a layer, you can create loss tensors that you will want to use later,
when writing your training loop. This is doable by calling
`self$add_loss(value)`:
...
These losses are meant to be taken into account when writing custom
training loops.

They also work seamlessly with
[`fit()`](https://generics.r-lib.org/reference/fit.html) (they get
...
```
    ...
    ## 1/1 - 0s - 109ms/step - loss: 1.6613

## You can optionally enable serialization on your layers

If you need your custom layers to be serializable as part of a
[Functional model](https://keras3.posit.co/articles/functional_api.md),
...
keyword arguments, in particular a `name` and a `dtype`. It’s good
practice to pass these arguments to the parent class in `initialize()`
...
config, you can also override the
[`from_config()`](https://keras3.posit.co/reference/get_config.md) class
...
(boolean) argument in the [`call()`](https://rdrr.io/r/base/call.html)
method.

By exposing this argument in
[`call()`](https://rdrr.io/r/base/call.html), you enable the built-in
training and evaluation loops
(e.g. [`fit()`](https://generics.r-lib.org/reference/fit.html)) to
...
The other privileged argument supported by
[`call()`](https://rdrr.io/r/base/call.html) is the `mask` argument.
...
Keras will automatically pass the correct `mask` argument to
[`call()`](https://rdrr.io/r/base/call.html) for layers that support it,
when a mask is generated by a prior layer. Mask-generating layers are
the `Embedding` layer configured with `mask_zero = TRUE`, and the
`Masking` layer.
...
- It exposes saving and serialization APIs
  ([`save()`](https://rdrr.io/r/base/save.html), `save_weights()`…)
...
class?”, ask yourself: will I need to call
[`fit()`](https://generics.r-lib.org/reference/fit.html) on it? Will I
need to call [`save()`](https://rdrr.io/r/base/save.html) on it? If so,
go with `Model`. If not (either because your class is just a block in a
bigger system, or because you are writing training & saving code
yourself), use `Layer`.

For instance, we could take our mini-resnet example above, and use it to
build a `Model` that we could train with
[`fit()`](https://generics.r-lib.org/reference/fit.html), and that we
could save with `save_weights()`:
...
- A `Layer` encapsulate a state (created in `initialize()` or `build()`)
  and some computation (defined in
  [`call()`](https://rdrr.io/r/base/call.html)).
- Layers can be recursively nested to create new, bigger computation
  blocks.
- Layers are backend-agnostic as long as they only use Keras APIs. You
  can use backend-native APIs (such as `jax$numpy`, `torch$nn` or
  `tf$nn`), but then your layer will only be usable with that specific
  backend.
...
going to implement a Variational AutoEncoder (VAE) in a backend-agnostic
fashion – so that it runs the same with TensorFlow, JAX, and PyTorch.
...
Let’s train it on MNIST using the
[`fit()`](https://generics.r-lib.org/reference/fit.html) API:
