...
- The model’s compilation information (if
  [`compile()`](https://generics.r-lib.org/reference/compile.html) was
  called)
...
You can save a model with
[`save_model()`](https://keras3.posit.co/reference/save_model.md). You
can load it back with
...
Layer, you **must** define a
[`get_config()`](https://keras3.posit.co/reference/get_config.md) method
on the object class. If the arguments passed to the constructor
(`initialize()` method) of the custom object aren’t simple objects
(anything other than types like ints, strings, etc.), then you **must**
also explicitly deserialize these arguments in the
...
on a custom object registers the object globally in a master list,
...
# the class name.
layer_custom <- register_keras_serializable(layer_custom, package = "MyLayers")
...
- [`get_config()`](https://keras3.posit.co/reference/get_config.md) and
  `cls.from_config()`: retrieve the configuration of a layer or model,
  and recreate a model instance from its config, respectively.
...
- `keras.saving.deserialize_keras_object()`: recreate an object instance
...
This is equivalent to getting the config then recreating the model from
its config (so it does not preserve compilation information or layer
weights values).
...
Calling `get_config(model)` or `get_config(layer)` will return a named
list containing the configuration of the model or layer, respectively.
You should define
[`get_config()`](https://keras3.posit.co/reference/get_config.md) to
contain arguments needed for the `initialize()` method of the model or
layer. At loading time, the `from_config(config)` method will then call
`initialize()` with these arguments to reconstruct the model or layer.
...
Now let’s reconstruct the layer using the
...
model into a JSON file, which can then be loaded without the original
...
APIs are general-purpose APIs that can be used to serialize or
deserialize any Keras object and any custom object. It is at the
foundation of saving model architecture and is behind all
[`serialize()`](https://rdrr.io/r/base/serialize.html)/`deserialize()`
calls in keras.
...
- `module` containing the name of the Keras module or other identifying
  module the object comes from
...
[`get_weights()`](https://keras3.posit.co/reference/get_weights.md) and
[`set_weights()`](https://keras3.posit.co/reference/get_weights.md):

- `get_weights(<layer>)`: Returns a list of arrays of weight values.
- `set_weights(<layer>weights)`: Sets the model/layer weights to the
  values provided (as arrays).
...
***Transferring weights from one model to another model with a
...
may result in a different output from `get_weights(layer)` ordering when
the model contains nested layers.
...
- [`get_config()`](https://keras3.posit.co/reference/get_config.md)
  should return a JSON-serializable named list in order to be compatible
  with the Keras architecture and model-saving APIs.
- `from_config(config)` (a class method) should return a new layer or
...
[`from_config()`](https://keras3.posit.co/reference/get_config.md) is
not necessary. However, for more complex objects such as layers or
models passed to `initialize()`, deserialization must be handled
explicitly either in `initialize` itself or overriding the
[`from_config()`](https://keras3.posit.co/reference/get_config.md)
method.
...
`initialize()`, for example, you must explicitly deserialize these
objects. Let’s take a look at an example of a model where a
`from_config` override is necessary.
...
layer_custom_model <- register_keras_serializable(
  ...
  object = Layer(
    ...
    from_config = function(config) {
      ...
      # `from_config()` also provides `cls` for the class; these are equivalent:
      # - CustomModel(!!!config)
      # - `__class__`(!!!config)
      # - cls(!!!config)
      cls(!!!config)
...
via
[`register_keras_serializable()`](https://keras3.posit.co/reference/register_keras_serializable.md).
This `registered_name` key allows for easy retrieval at
...
name `KernelMult` that we gave when calling
`register_keras_serializables()`. Take a look again at the custom class
definition/registration [here](#registration_example).

Note that the `class_name` key contains the original name of the class,
allowing for proper re-initialization in `from_config`.

Additionally, note that the `module` key is `NULL` since this is a
custom object.
