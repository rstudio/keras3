...
![](functional_api/unnamed-chunk-10-1.png)
...
![](functional_api/unnamed-chunk-11-1.png)

This figure and the code are almost identical. In the code version, the
connection arrows are replaced by the call operation.
...
[`fit()`](https://generics.r-lib.org/reference/fit.html) method) and a
built-in evaluation loop (the
[`evaluate()`](https://rdrr.io/pkg/tensorflow/man/evaluate.html)
method). Note that you can easily [customize these
loops](https://keras3.posit.co/articles/custom_train_step_in_tensorflow.md)
...
standard way to save a functional model is to call `model.save()` to
...
saving](https://keras3.posit.co/articles/serialization_and_saving.md)
guide.
...
The reverse of a `conv_2d` layer is a `conv_2d_transpose` layer, and the
reverse of a `max_pooling_2d` layer is an `upsampling_2d` layer.
...
- the department that should handle the ticket (softmax output over the
  set of departments).
...
![](functional_api/unnamed-chunk-20-1.png)
...
![](functional_api/unnamed-chunk-25-1.png)
...
- Convolutional layers: `conv_1d`, `conv_2d`, `conv_3d`,
  `conv_2d_transpose`
- Pooling layers: `max_pooling_1d`, `max_pooling_2d`, `max_pooling_3d`,
  `average_pooling_3d`
...
- `build` method, that creates the weights of the layer (this is just a
  style convention since you can create weights in `initialize`, as
  well).

To learn more about creating layers from scratch, read [custom layers
and
models](https://keras3.posit.co/articles/making_new_layers_and_models_via_subclassing.md)
guide.
...
For serialization support in your custom layer, define a
...
R and Python (byte)code, not data structures).

#### Less verbose

There is no `super$initialize(...)`, no `call = function(...)`, no
`self$...`, etc.
...
created in advance (using `Input`). Every time you call a layer, the
...
guide](https://keras3.posit.co/articles/serialization_and_saving.md).
...
the `keras` API can interact with each other, whether they’re
...
- `call(inputs, ...)` – Where `inputs` is a tensor or a nested structure
  of tensors (e.g. a list of tensors), and where `...` are non-tensor
  arguments (non-inputs).
- `call(inputs, training = NULL, ...)` – Where `training` is a boolean
  indicating whether the layer should behave in training mode and
  inference mode.
- `call(inputs, mask = NULL, ...)` – Where `mask` is a boolean mask
  tensor (useful for RNNs, for instance).
- `call(inputs, training = NULL, mask = NULL, ...)` – Of course, you can
  have both masking and training-specific behavior at the same time.
