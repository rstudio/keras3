...
For more information, see the documentation for [multi_gpu_model](https://tensorflow.rstudio.com/reference/keras/multi_gpu_model.html). Here is a quick example:
...
<!-- You can see batch image training in action in our [CIFAR10 example](https://tensorflow.rstudio.com/articles/examples/cifar10_cnn.html). -->
...
The VGG16 model is also the basis for the [Deep dream](https://tensorflow.rstudio.com/examples/deep_dream) Keras example script.
...
For example, here's a [testthat](https://r-pkgs.org/testing-basics.html) utility function that can be used to skip a test when Keras isn't available:
...
Since the Keras R package can bind to multiple different implementations of Keras (either the original Keras or the TensorFlow implementation of Keras), you should use the `keras3::implementation()` function to obtain access to the correct python module. You can use this function within the `.onLoad` function of a package to provide global access to the module within your package. For example:
